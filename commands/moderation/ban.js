const { Message, ActionRowBuilder, ButtonBuilder, ButtonStyle, ChatInputCommandInteraction, EmbedBuilder, PermissionFlagsBits } = require("discord.js");
const { msgDelete } = require("../../funcsSuporte/message");
...
module.exports = {
    ...
    async execute(msg) {
        ...
        collector.on("collect", async (message) => {
            ...
            for (const i of message.content.split("\n")) {
                ...
                try {
                    ...
                    if (member?.roles?.highest.position >= client.roles?.highest.position) {
                        let msgM = await msg.channel?.send({ content: `Não consigo banir o membro ${member}` });
                        await msgDelete(msgM, 3000)
                    } else {
                        desc += `${member}\n`
                    }
            ...
            if (desc.length == 0) {
                await fs.edit({ content: "Não consegui banir ninguem" });
                await msgDelete(fs, 0)
                return
            }
